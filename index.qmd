...
To replicate the examples, you'll need the data here [sample_data.zip](sample_data.zip) and to setup your `snakemake` environment as described [here](https://bioinformatics.tsl.ac.uk/posts/a-downloadable-template-for-snakemake-projects/), which will set you up with the latest release.  This step is vital as we will be using the script `run_workflow.py` during this tutorial.  Once you have setup your snakemake environment you can use the script `install_ok.py` within the `test` directory to confirm everything is setup as required.
