...
Where `profiles` is a directory containing your workflow profile, which needs to be named "config.v9+.yaml" in order for snakemake to recognise it as a profile and utilise the defaults as expected.
...
This is the most important step before beginning working with our `snakemake` pipeline.
